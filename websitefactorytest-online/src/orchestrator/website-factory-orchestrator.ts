...
export interface OrchestratorInput {
   ...
   * Constructed at CLI entry point. Passed through to domain.config.json.
   */
...
// ---------------------------------------------------------------------------
export function writeDomainConfig(siteUrl: string, domain: string, identity: ProjectIdentity): void {
...
export async function runWebsiteFactory(
  ...
  const { siteUrl, domain, mode, identity } = input;
  ...
  if (!stopped) {
    const cms = await runStage("cms-generator", () => generateCms(siteUrl, identity));
