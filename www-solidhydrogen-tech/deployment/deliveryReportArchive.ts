...
function getReportsDir(): string {
  return path.join(getContext().workspaceRoot, "reports");
}
...
function getGitInfo(): { commit: string; branch: string } {
  ...
  try {
    const headPath = path.join(getContext().workspaceRoot, ".git", "HEAD");
...
export function buildArchivedReport(
  ...
  return {
    ...
    targetWebsite: getContext().referenceWebsite,
    targetDomain: state.domain,
    environment: state.environment,
    github: {
      repository: getContext().githubRepository,
...
export function writeReportToArchive(
  ...
  log.info(
    "archive",
    "writeReport",
    `report written: ${path.relative(getContext().workspaceRoot, finalPath)}`,
  );
