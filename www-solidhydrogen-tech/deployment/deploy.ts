...
async function stepPushCode(
  ...
  return runStep(step.step, step.id, step.provider, dryRun, async () => {
    ...
    const pushResult = await GitHub.pushFolderCode({
      ...
      branch: "main",
      message: `deploy: ${state.domain} → ${state.projectFolder}`,
      sourceDir: ".",
      folderName: state.projectFolder,
    });
...
async function stepCreateProject(
  ...
  return runStep(step.step, step.id, step.provider, dryRun, async () => {
    ...
    const result = await Vercel.createProject({
      ...
      rootDirectory: state.projectFolder,
      teamId: "glotalk",
    });
    ...
    if (result.success && result.data) {
      ...
      throw new Error(`createProject failed for "${state.projectSlug}": ${result.error}`);
    }
  });
}
