...
export function generateCmsManifest(siteUrl: string): CmsManifestOutput {
...
export function generateCmsSearchOutput(siteUrl: string): CmsSearchOutput {
...
export function generateCmsNavigation(siteUrl: string): CmsNavigationOutput {
  ...
  if (homepage) {
    mainNav.push({ label: "Home", href: homepage.url.replace(siteUrl, "") || "/" });
  }

  const aboutPage = pages.find(p => p.page_type === "about");
  if (aboutPage) {
    mainNav.push({ label: "About", href: aboutPage.url.replace(siteUrl, "") || "/about" });
  }
  ...
  if (blogListing) {
    mainNav.push({ label: "Blog", href: blogListing.url.replace(siteUrl, "") || "/blog" });
  }

  const contactPage = pages.find(p => p.page_type === "contact");
  if (contactPage) {
    mainNav.push({ label: "Contact", href: contactPage.url.replace(siteUrl, "") || "/contact" });
  }
...
export function generateCmsSitemap(siteUrl: string): CmsSitemapOutput {
  const pages = db.prepare(
