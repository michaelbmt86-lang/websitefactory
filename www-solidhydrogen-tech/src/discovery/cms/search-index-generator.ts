...
export function generateSearchIndex(siteUrl: string): {
  ...
  for (const page of pages) {
    ...
    insertStmt.run("page", page.id, page.title, page.description, keywords, page.url, page.og_image || "", page.page_type);
    indexedEntities++;
  }
  ...
  for (const product of products) {
    ...
    insertStmt.run(
      ...
      keywords, product.url, primaryImage, product.category
    );
    indexedEntities++;
  }
  ...
  for (const brand of brands) {
    ...
    insertStmt.run("brand", brand.id, brand.name, brand.description, keywords, `${siteUrl}/brands/${brand.slug}`, brand.logo_url || "", "brand");
    indexedEntities++;
  }
  ...
  for (const collection of collections) {
    ...
    insertStmt.run("collection", collection.id, collection.name, collection.description, keywords, `${siteUrl}/collections/${collection.slug}`, collection.image_url || "", "collection");
    indexedEntities++;
  }
  ...
  for (const blog of blogPages) {
    ...
    insertStmt.run("blog", blog.id, blog.title, blog.description, keywords, blog.url, blog.og_image || "", "blog");
    indexedEntities++;
  }
