...
async function verifyPreflight(): Promise<VerificationCheck[]> {
  ...
  checks.push(await runCheck(0, "context_tokens", "runtime", async () => {
    if (!Context.githubToken) throw new Error("GITHUB_TOKEN missing");
    if (!Context.vercelToken) throw new Error("VERCEL_TOKEN missing");
    if (!Context.cloudflareToken) throw new Error("CLOUDFLARE_API_TOKEN missing");
  }));

  checks.push(await runCheck(0, "context_domain", "runtime", async () => {
    if (!Context.targetDomain) throw new Error("targetDomain is empty");
    if (!Context.deploymentProvider) throw new Error("deploymentProvider is empty");
    if (!Context.dnsProvider) throw new Error("dnsProvider is empty");
  }));

  checks.push(await runCheck(0, "config_files", "runtime", async () => {
    const deploymentRoot = Context.deploymentRoot;
    const required = [
      "domain.config.json",
      "vercel.config.json",
  ...
  checks.push(await runCheck(0, "workspace_root", "runtime", async () => {
    if (!dirExists(Context.workspaceRoot)) {
      throw new Error(`workspaceRoot not found: ${Context.workspaceRoot}`);
    }
    if (!dirExists(path.join(Context.workspaceRoot, "src"))) {
      throw new Error("src/ directory missing in workspace");
    }
    if (!fileExists(path.join(Context.workspaceRoot, "package.json"))) {
...
async function verifyGitHub(): Promise<VerificationCheck[]> {
  const checks: VerificationCheck[] = [];
  const repoFullName = `${Context.githubRepository}`;
...
async function verifyCloudflare(): Promise<VerificationCheck[]> {
  const checks: VerificationCheck[] = [];
  const domain = Context.targetDomain;
...
export async function verifyDeployment(
  ...
  log.info("verify", "verifyDeployment", `starting verification for ${Context.targetDomain}`);
