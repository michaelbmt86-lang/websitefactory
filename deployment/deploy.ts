...
async function stepCreateProject(
  ...
  return runStep(step.step, step.id, step.provider, dryRun, async () => {
    ...
    const { projectId, projectName } = await createProjectWithUniqueName(
      state.projectSlug,
      vConfig.framework,
    );

    state.projectId = projectId;
    state.vercelProjectName = projectName;
  });
}
...
async function stepConnectGithub(
  ...
  return runStep(step.step, step.id, step.provider, dryRun, async () => {
    ...
    const linkResult = await Vercel.connectGithub(state.projectId, state.repoFullName, "main");
...
async function stepConfigureEnv(
  ...
  return runStep(step.step, step.id, step.provider, dryRun, async () => {
    const envResult = await Vercel.setEnvironmentVariables(state.projectId, {
      ...
      ADMIN_PASSWORD: Context.defaultAdminPassword,
    });
...
async function stepBindDomain(
  ...
  return runStep(step.step, step.id, step.provider, dryRun, async () => {
    ...
    const wwwDomain = `www.${state.domain}`;
  ...
  return runStep(step.step, step.id, step.provider, dryRun, async () => {
    ...
    await retry(
      async () => {
        ...
        const rootDomain = state.domain.split(".").slice(-2).join(".");
        const apexName = state.domain.replace(`.${rootDomain}`, "") || "@";
        const wwwName = "www";
...
async function stepVerifyHttps(
  ...
  return runStep(step.step, step.id, step.provider, dryRun, async () => {
    const urls = [`https://${state.domain}`, `https://www.${state.domain}`];
...
async function stepVerifyDashboardLogin(
  ...
  return runStep(step.step, step.id, step.provider, dryRun, async () => {
    const loginUrl = `https://${state.domain}/login`;
...
async function stepVerifyAdminAccount(
  ...
  return runStep(step.step, step.id, step.provider, dryRun, async () => {
    const apiUrl = `https://${state.domain}/api/auth/me`;
...
async function stepDeliveryComplete(
  ...
  return runStep(step.step, step.id, step.provider, dryRun, async () => {
    ...
    checks.push(await deliveryCheck(5, "custom_domain_bound", async () => {
      if (!state.projectId || !state.domain) throw new Error("projectId or domain not set");
    ...
    checks.push(await deliveryCheck(6, "https_apex_200", async () => {
      const resp = await fetch(`https://${state.domain}`, { redirect: "follow" });
      if (!resp.ok) throw new Error(`https://${state.domain} returned HTTP ${resp.status}`);
    }));

    // ── Check 7: HTTPS www 200 ─────────────────────────────────────────────
    checks.push(await deliveryCheck(7, "https_www_200", async () => {
      const resp = await fetch(`https://www.${state.domain}`, { redirect: "follow" });
      if (!resp.ok) throw new Error(`https://www.${state.domain} returned HTTP ${resp.status}`);
    }));
    ...
    checks.push(await deliveryCheck(11, "dashboard_200", async () => {
      const resp = await fetch(`https://${state.domain}/dashboard`, { redirect: "follow" });
    ...
    checks.push(await deliveryCheck(12, "dashboard_login_works", async () => {
      const resp = await fetch(`https://${state.domain}/login`, { redirect: "follow" });
    ...
    checks.push(await deliveryCheck(13, "admin_account_exists", async () => {
      const resp = await fetch(`https://${state.domain}/api/auth/me`);
    ...
    checks.push(await deliveryCheck(14, "admin_login_succeeds", async () => {
      const loginResp = await fetch(`https://${state.domain}/api/auth/login`, {
        method: "POST",
    ...
    checks.push(await deliveryCheck(15, "admin_dashboard_loads", async () => {
      ...
      const resp = await fetch(`https://${state.domain}/dashboard`, {
    ...
    checks.push(await deliveryCheck(16, "no_not_found_errors", async () => {
      const urls = [
        `https://${state.domain}/about`,
        `https://${state.domain}/products`,
        `https://${state.domain}/contact`,
        `https://${state.domain}/blog`,
        `https://${state.domain}/login`,
        `https://${state.domain}/dashboard`,
      ];
      for (const url of urls) {
        const resp = await fetch(url, { redirect: "follow" });
    ...
    checks.push(await deliveryCheck(17, "no_404_pages", async () => {
      ...
      for (const p of criticalPaths) {
        const resp = await fetch(`https://${state.domain}${p}`, { redirect: "follow" });
    ...
    checks.push(await deliveryCheck(20, "cloudflare_dns_verified", async () => {
      ...
      if (!zone.data) throw new Error(`Cloudflare zone not found for ${state.domain}`);
    }));
...
function buildDeliveryReport(
  ...
  return {
    ...
    productionUrl: `https://${state.domain}`,
    dashboardUrl: `https://${state.domain}/dashboard`,
    adminUsername: Context.defaultAdminEmail,
    adminPassword: Context.defaultAdminPassword,
